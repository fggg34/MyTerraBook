...
const CARD_LAYOUT = [
  { accent: 'accent-green', layout: 'widget' },
  { accent: 'accent-navy', layout: 'bleed' },
  { accent: 'accent-navy', layout: 'photo' },
    ...
    <section className="host-feat" id="feat" ref={sectionRef}>
      <div className="wrap">
        <div className="host-feat-head">
          ...
          {subheading && <p className="host-feat-sub">{subheading}</p>}
        </div>
        <div className="host-feat-grid">
          {features.slice(0, 4).map((feature, index) => {
            const layout = CARD_LAYOUT[index] ?? CARD_LAYOUT[0]
            const imgSrc = feature.image || null
            const imgAlt = feature.imageAlt || feature.title || ''

            if (layout.layout === 'widget') {
              return (
                <div
                  key={`${feature.title}-${index}`}
                  className={`host-feat-card ${layout.accent} host-feat-card--widget${imgSrc ? ' host-feat-card--widget-photo' : ''}`}
                >
                  {imgSrc ? (
                    <div className="host-feat-widget-photo">
            ...
            if (layout.layout === 'photo') {
              return (
                <div key={`${feature.title}-${index}`} className={`host-feat-card ${layout.accent} host-feat-card--photo`}>
                  <HostPhoto src={imgSrc} alt={imgAlt} />
                  <div className="host-feat-card-body">
                    <h3>{feature.title}</h3>
                    <p>{feature.text}</p>
            ...
            if (layout.layout === 'bleed') {
              return (
                <div key={`${feature.title}-${index}`} className={`host-feat-card ${layout.accent} host-feat-card--bleed`}>
                  <div className="host-feat-card-body">
                    <h3>{feature.title}</h3>
                    <p>{feature.text}</p>
                  </div>
            ...
            return (
              <div key={`${feature.title}-${index}`} className={`host-feat-card ${layout.accent} host-feat-card--stats`}>
